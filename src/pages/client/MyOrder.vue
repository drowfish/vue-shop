      ...
      <ul class="orderList">
        <li v-for="(item,index) in orderList" :key="'order'+item.id">
          ...
          <div class="orderDetail">
            <img :src="baseimgurl+item.goodsInfo.img" alt="商品图片" />
            <div class="goodsName">
              <p @click="navTo('/mall/goods/'+item.goodsInfo.id)">{{item.goodsInfo.name}}</p>
              <span>{{item.goodsDetail.specname}}</span>
            </div>
            <span class="unitPrice">{{'￥'+item.goodsDetail.unitprice}}</span>
            ...
            <button v-else-if="item.state===3 && !item.hasComment" @click="showPopup(item.id,item.goodsInfo.id,item.goodsDetail.id)">评价</button>
...
import {getOrderByState,settleAccounts,sendComment} from '../../api/client';
...
export default {
  ...
  methods:{
    ...
    confirmPay(orderId){
      let cartList = [{id: orderId,state: 1}];
    ...
    confirmReceive(orderId){
      let cartList = [{id: orderId,state: 3}];
      const res = settleAccounts({cartList: cartList});
      res
      .then(()=>{
        alert('确认收货成功！');
