  	...
  	<div class="content">
  		<ul class="msgList" v-if="curIndex===0">
  			<li v-for="(item,index) in noReplyMsgList" :key="'noReply'+item.id" class="clear">
  				<img :src="baseimgurl+item.user.headimg" alt="" />
  				<div class="info">
  					<span class="name">{{item.user.nickname}}</span>
  					<div class="goods ellipsis">商品：{{item.goodsInfo.name}}</div>
  					<p>{{item.content}}</p>
  				</div>
  				<div class="operate">
  		...
  		<ul class="msgList" v-else="curIndex===1">
  			<li v-for="(item,index) in repliedMsgList" :key="'replied'+item.id" class="clear">
  				...
  				<div class="info">
  					...
  					<div class="goods ellipsis">商品：{{item.goodsInfo.name}}</div>
  					<p>{{item.content}}</p>
  					<p class="replyContent">{{'回复内容：'+item.reply.content}}</p>
  				</div>
  				<div class="operate">
  					<div>{{item.createtime}}</div>
  					<span>已回复</span>
...
<script>
import {getNoReplyMsg,getRepliedMsg,reply,updateMessage} from '../../api/admin';
...
export default {
  ...
  methods:{
      ...
      updateMessage({id: this.curMsgId,state: 1});
  		const res = reply({
